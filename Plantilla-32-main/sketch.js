...
function keyReleased() {
  if (keyCode === 32) {
    // Llamar a la función shoot() para cada flecha en el array playerArrows
...
function handlePlayerArrowCollision() {
  for (var i = 0; i < playerArrows.length; i++) {
    if (
      playerArrows[i].body.position.x >= width - computerBase.width &&
      playerArrows[i].body.position.x <= width &&
      playerArrows[i].body.position.y >= computerBase.body.position.y - computerBase.height / 2 &&
      playerArrows[i].body.position.y <= computerBase.body.position.y + computerBase.height / 2
    ) {
      // Colisión entre la flecha del jugador y la base de la computadora
      ...
      if (computerArcherLife === 0) {
        ...
        // Aquí puedes agregar más lógica o mostrar un mensaje al usuario
      }
    }
  }
}
...
function handleComputerArrowCollision() {
  for (var i = 0; i < computerArrows.length; i++) {
    if (
      computerArrows[i].body.position.x >= 0 &&
      computerArrows[i].body.position.x <= playerBase.width &&
      computerArrows[i].body.position.y >= playerBase.body.position.y - playerBase.height / 2 &&
      computerArrows[i].body.position.y <= playerBase.body.position.y + playerBase.height / 2
    ) {
      // Colisión entre la flecha de la computadora y la base del jugador
      computerArrows[i].remove();
